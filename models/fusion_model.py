...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 agent_num=agent_N, attn_type= attn):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
...
class Cross_SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, agent_num=agent_N, attn_type=attn):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.num_heads = num_heads
        ...
        assert attn_type in ['A', 'B']
        if attn_type == 'A':
            self.attn_A = Cross_AgentAttention(
            dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,agent_num=agent_num,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
            
            self.attn_B = Cross_AgentAttention(
            dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,agent_num=agent_num,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
...
class CRSTB(nn.Module):
    """Cross Residual Swin Transformer Block (RSTB).
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.residual_group = Cross_BasicLayer(dim=dim,
                                         ...
                                         downsample=downsample,
                                         use_checkpoint=use_checkpoint)
        # self.residual_group_Feature = BasicLayer(dim=dim,
        #                                  input_resolution=input_resolution,
        #                                  depth=depth,
        #                                  num_heads=num_heads,
        #                                  window_size=window_size,
    ...
    def forward(self, x, y, x_size):
        ## Intra-Modal Fusion  sigma3 add
        # x = self.residual_group_Feature(x, x_size) + x
        # y = self.residual_group_Feature(y, x_size) + y
        ## Inter-Modal Fusion
        x1 = x
        y1 = y
...
class LACT_Fusion(nn.Module):
    def __init__(self, img_size=64, patch_size=1, in_chans=1,
                 embed_dim=60, Ex_depths=[1], Fusion_depths=[1], Re_depths=[1], 
                 Ex_num_heads=[6, 6], Fusion_num_heads=[6, 6], Re_num_heads=[6],
                 window_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, upscale=2, img_range=1., resi_connection='1conv',
                 **kwargs):
        super(LACT_Fusion, self).__init__()
        num_out_ch = in_chans
        # num_feat = 64
        ...
        self.conv_first1_A = nn.Sequential(
                                          nn.Conv2d(in_chans, embed_dim_temp, 3, stride=1, padding=1),
                                          nn.LeakyReLU(0.2, inplace=True),
                                          nn.Conv2d(embed_dim_temp, embed_dim, 3, stride=1, padding=1),
                                          nn.LeakyReLU(0.2, inplace=True)
                                           )
   
      
        self.lattn_A = DetailAwareAttention(embed_dim)
        
    
        self.conv_first1_B = nn.Sequential(
                                          nn.Conv2d(in_chans, embed_dim_temp, 3, stride=1, padding=1),
                                          nn.LeakyReLU(0.2, inplace=True),
                                          nn.Conv2d(embed_dim_temp, embed_dim, 3, stride=1, padding=1),
                                          nn.LeakyReLU(0.2, inplace=True)
                                           )
        self.lattn_B = DetailAwareAttention(embed_dim)
        ...
        self.layers_Ex_Feature = nn.ModuleList()
        for i_layer in range(self.Ex_num_layers):
            layer = RSTB(dim=embed_dim,
                         ...
                         drop_path= drop_path_rate, #dpr_Ex[sum(Ex_depths[:i_layer]):sum(Ex_depths[:i_layer + 1])],  # no impact on SR results
                         norm_layer=norm_layer,
                         downsample=None,
                         use_checkpoint=use_checkpoint,
                         img_size=img_size,
            ...
            self.layers_Ex_Feature.append(layer)
        self.norm_Ex_Feature = norm_layer(self.num_features)
        ...
        for i_layer in range(self.Fusion_num_layers):
            layer = CRSTB(dim=embed_dim,
                         ...
                         qkv_bias=qkv_bias, qk_scale=qk_scale,
                         drop=drop_rate, attn_drop=attn_drop_rate,
                         drop_path=drop_path_rate, #dpr_Fusion[sum(Fusion_depths[:i_layer]):sum(Fusion_depths[:i_layer + 1])],  # no impact on SR results
                         norm_layer=norm_layer,
                         downsample=None,
                         use_checkpoint=use_checkpoint,
                         img_size=img_size,
    ...
    def forward_features_Ex_Feature_A(self, x):
        x = self.conv_first1_A(x)
        x = self.lattn_A(x)
        x_size = (x.shape[2], x.shape[3])       
        x = self.patch_embed(x)    # [16,16384,60]
        if self.ape:
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x)

        for layer in self.layers_Ex_Feature:
            x = layer(x, x_size)

        x = self.norm_Ex_Feature(x)  # B L C
        x = self.patch_unembed(x, x_size)

        return x
    
    def forward_features_Ex_Feature_B(self, x):
        x = self.conv_first1_B(x)
        x = self.lattn_B(x)  
        x_size = (x.shape[2], x.shape[3])       
    ...
    def forward(self, A, B):
        ...
        # Feedforward
        x = self.forward_features_Ex_Feature_A(x)
        y = self.forward_features_Ex_Feature_B(y)
...
def build_model(config):
    model = LACT_Fusion(upscale=2, img_size=config.img_size, embed_dim=60,
                   window_size=config.window_size, img_range=1.,num_heads=[6, 6, 6, 6],
                   mlp_ratio=2)
...
if __name__ == "__main__":
    ...
    window_size = 8
    img_size = (H, W)
    model = LACT_Fusion(upscale=2, img_size=(H, W),
                   window_size=window_size, img_range=1.,
                   embed_dim=64, num_heads=[6, 6, 6, 6], mlp_ratio=2)
    # model = LACT_Fusion(upscale=2, img_size=(H, W),
    #                window_size=window_size, img_range=1.,
    #             mlp_ratio=2)
    y = model(x, x)
